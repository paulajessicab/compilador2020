...
desugar (SUnaryOp p op t)  = UnaryOp p op (desugar t)
...
--desugar (SLet p v [(x, tx)] ty t t') = desugar $ SLet p v [] (FunTy tx ty) (SLam p x tx t) t'
...
elab' (Fix p f fty x xty t) = Fix p f fty x xty (closeN [let f, x] (elab' t))
