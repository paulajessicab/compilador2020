...
void run(code init_c)
{
	...
	while (1) {
		...
		case IFZ: {
			/* Tomo del stack el valor de la condicion */
			int cond = (*--s).i;

			/* Consumo la longitud de la primer rama mas el JMP */
			int lenT0 = *c++;
			
			/*Si es 0 no necesito hacer nada porque se ejecuta la condicion y salta*/
			if(cond != 0)
			{
			  c += lenT0;
			}
